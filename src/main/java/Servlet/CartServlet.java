        ...
        try (Connection connection = DBConnection.getConnection()) {
            ...
            try {
                ...
                if (product == null || !product.getStatus()) {
                    logger.warn("Product not found or inactive for ID: {}, IP: {}", productId, remoteIp);
                ...
                switch (action) {
                    case "add":
                        ...
                        if (addQuantity >= 1 && addQuantity <= 100 && addQuantity <= product.getStock()) {
                            ...
                            logger.warn("Invalid quantity: {} for product ID: {}, stock: {}, IP: {}", addQuantity, productId, product.getStock(), remoteIp);
                    ...
                    case "edit":
                        ...
                        if (updatedQuantity >= 1 && updatedQuantity <= 100 && updatedQuantity <= product.getStock()) {
                            ...
                            logger.warn("Invalid quantity: {} for product ID: {}, stock: {}, IP: {}", updatedQuantity, productId, product.getStock(), remoteIp);
                            session.setAttribute("error", "Số lượng không hợp lệ hoặc vượt quá tồn kho.");
                            response.sendRedirect("cart.jsp");
                            return;
                        }
                ...
                logger.error("Database error while processing cart action: {}, productId: {}, IP: {}, error: {}", action, productIdStr, remoteIp, e.getMessage());
                session.setAttribute("error", "Lỗi cơ sở dữ liệu.");
                response.sendRedirect("error.jsp");
            } catch (NumberFormatException e) {
                logger.error("Invalid number format for productId: {} or quantity: {}, IP: {}, error: {}", productIdStr, quantityStr, remoteIp, e.getMessage());
