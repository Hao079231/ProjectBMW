...
<body class="font-roboto bg-gray-100">
    <% String message=(String) session.getAttribute("message"); String error=(String)
        session.getAttribute("error"); // Xóa thông báo khỏi session sau khi hiển thị
        session.removeAttribute("message"); session.removeAttribute("error"); %>

        <!-- Thông báo thành công -->
        <% if (message !=null) { %>
            <div id="success-toast"
                class="bg-green-600 text-white p-4 rounded-lg shadow-lg mb-6 flex items-center space-x-4">
                <i class="fas fa-check-circle text-xl"></i>
                <span>
                    <%= message %>
                </span>
                <button onclick="closeToast('success-toast')"
            ...
            <% } %>
                ...
                <% if (error !=null) { %>
                    <div id="error-toast"
                        class="bg-red-600 text-white p-4 rounded-lg shadow-lg mb-6 flex items-center space-x-4">
                            ...
                            <div class="container mx-auto flex justify-between items-center py-4 px-6">
                                ...
                                <div class="flex items-center space-x-6">
                                    ...
                                    if (session.getAttribute("user") !=null) {
                                        ...
                                        <a href="<%= profileLink %>">
                                            <i
                                                class="fas fa-user text-gray-600 text-lg cursor-pointer"></i>
                                        </a>

                                        <i
                                            class="fas fa-heart text-gray-600 text-lg cursor-pointer"></i>
                        ...
                        <div class="max-w-7xl mx-auto px-4 py-6" id="main-content">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"
                                ...
                                <% List<Orders> ordersList = (List<Orders>)
                                        request.getAttribute("ordersList"); // Lấy danh sách đơn hàng
                                        if (ordersList != null && !ordersList.isEmpty()) {
                                        for (Orders order : ordersList) {
                                        %>
                                        <div class="bg-white p-6 rounded-lg shadow-lg">
                                            <h2 class="text-xl font-bold mb-2">Mã đơn hàng: <%=
                                                    order.getOrderId() %>
                                            </h2>
                                            <p class="text-gray-700 mb-4">Tổng tiền: <%=
                                                    order.getTotalAmount() %>
                                            </p>
                                            <p class="text-gray-700 mb-4">Trạng thái thanh toán: <%=
                                                    order.isPaymentStatus() ? "Đã thanh toán"
                                                    : "Chưa thanh toán" %>
                                            </p>
                                            <p class="text-gray-700 mb-4">Trạng thái giao hàng: <%=
                                                    order.getDeliveryStatus() %>
                                            </p>
                                            <p class="text-gray-700 mb-4">Ngày tạo: <%=
                                                    order.getCreatedAt() %>
                                            </p>
                                            <p class="text-gray-700 mb-4">Ngày cập nhật: <%=
                                                    order.getUpdatedAt() %>
                                            </p>
                                            <!-- Cập nhật liên kết chi tiết đơn hàng -->
                                            <a href="CustomerOrder?order_id=<%= order.getOrderId() %>"
                                                class="bg-blue-500 text-white px-3 py-1 rounded">Chi
                                                tiết</a>

                                            <!-- Nút Đã nhận được hàng -->
                                            <% if (!order.isPaymentStatus()) { %>
                                                <!-- Kiểm tra nếu chưa thanh toán -->
                                                <form action="CustomerOrder" method="post" class="mt-4">
                                                    <input type="hidden" name="orderId"
                                                        value="<%= order.getOrderId() %>">
                                                    <button type="submit" name="action" value="received"
                                                        class="bg-green-500 text-white px-4 py-2 rounded">Đã
                                                        nhận được hàng</button>
                                                </form>
                                                <% } %>
                                        </div>
                                        <% } } else { %>
                                            <p class="text-center text-gray-500">Không có đơn hàng nào.
                                            </p>
                                            <% } %>
                            </div>
                        </div>
